...
let prods (ls : (int * int) list) : int list =
  List.map times ls ;;
...
let rec max_list (lst : int list) : int option =
  ...
  | [] -> None
  | [elt] -> Some elt
  | head :: tail -> (Pervasives.max (Some head) (max_list tail));;
...
let min_option (x : int option) (y : int option) : int option =
  ...
  | Some a -> match y with
              | None -> x
              | Some b -> Some (Pervasives.min a b) ;;
...
let max_option (x : int option) (y : int option) : int option =
  ...
  | Some a -> match y with
              | None -> x
              | Some b -> Some (Pervasives.max a b) ;;
...
let min_option_2 (x : int option) (y : int option) : int option =
  calc_option Pervasives.min x y ;;

let max_option_2 (x : int option) (y : int option) : int option =
  calc_option Pervasives.max x y ;;
